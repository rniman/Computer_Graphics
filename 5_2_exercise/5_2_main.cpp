...
GLuint pillar_vao;
GLuint pillar_vbo[2];
glm::mat4 pillar_trans[5];
glm::vec3 pillar_pos[5];
std::vector<GLfloat> pillar_vertex;
std::vector<GLfloat> pillar_normal;
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("Example_5_2(27번)");
	...
	makeCuboid(pillar_vertex, 40, 80);
	setNormal(pillar_normal);
	for (int i = 0; i < 5; ++i)
	{
		pillar_trans[i] = glm::mat4(1.0f);
		pillar_pos[i] = glm::vec3(dis(gen), 80.0f, dis(gen));
		pillar_trans[i] = glm::translate(pillar_trans[i], pillar_pos[i]);
	}
	...
	shaderID_02 = make_shaderProgram_not_texture();
	initBuffer();
	...
	modelLocation = glGetUniformLocation(shaderID_02, "modelTransform");
	viewLocation = glGetUniformLocation(shaderID_02, "viewTransform");
	projLocation = glGetUniformLocation(shaderID_02, "projectionTransform");
	lightPosLocation = glGetUniformLocation(shaderID_02, "lightPos"); //조명 위치
	lightColorLocation = glGetUniformLocation(shaderID_02, "lightColor"); //조명 색 설정
	objColorLocation = glGetUniformLocation(shaderID_02, "objectColor");  //주황색 반사
	viewPosLocation = glGetUniformLocation(shaderID_02, "cameraEye");     //--- viewPos 값 전달: 카메라 위치 
	ambientLocation = glGetUniformLocation(shaderID_02, "ambientLight");
	alphaLocation = glGetUniformLocation(shaderID_02, "alpha");
...
GLvoid drawScene()
{
	...
	//기둥
	glUniform3f(objColorLocation, 0.7, 0.4, 0.4);
	glUniform1f(alphaLocation, 0.1f);
	...
	glUniform3f(objColorLocation, 0.2, 0.8, 0.8);
	glBindVertexArray(pillar_vao);
	for (int i = 0; i < 5; ++i)
	{
		glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(pillar_trans[i]));
		glDrawArrays(GL_TRIANGLES, 0, pillar_vertex.size());
	}
	glutSwapBuffers();
}
...
void initBuffer()
{
	...
	//기둥
	glGenVertexArrays(1, &pillar_vao);
	glGenBuffers(2, pillar_vbo);

	glBindVertexArray(pillar_vao);

	glBindBuffer(GL_ARRAY_BUFFER, pillar_vbo[1]);
	glBufferData(GL_ARRAY_BUFFER, pillar_normal.size() * sizeof(GLfloat), pillar_normal.data(), GL_STATIC_DRAW);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(1);

	glBindBuffer(GL_ARRAY_BUFFER, pillar_vbo[0]);
	glBufferData(GL_ARRAY_BUFFER, pillar_vertex.size() * sizeof(GLfloat), pillar_vertex.data(), GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(0);
}
